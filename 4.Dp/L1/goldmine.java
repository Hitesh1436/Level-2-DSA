...
public class Main {
    ...
    // yeh fail hua bcz test cases heavy hn for recusrion
    ...
    // }
       ...
       // memoization approach hai yh
        public static int goldMine(int i,int j,int[][]grid,int[][]dp){
        if( i>=grid.length || j>=grid[0].length || i<0 ){
            return 0;
        }
        if(dp[i][j]!=0){
            return dp[i][j];
        ...
        for(int j=m-1;j>=0;j--){
            for(int i=0;i<n;i++){
                if(j==m-1){
                    ...
                    dp[i][j] = grid[i][j] + Math.max(op3 , op1);
                }
                else{
                    int op3 = dp[i-1][j+1];
                    int op1 = dp[i][j+1];
                    ...
                    dp[i][j] = grid[i][j] + Math.max(op3 , Math.max(op1 , op2));
